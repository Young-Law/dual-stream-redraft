...
def build_parser() -> argparse.ArgumentParser:
    ...
    exp = sub.add_parser("experiment", help="Run and analyze DSA experiments")
    exp_sub = exp.add_subparsers(dest="experiment_cmd", required=True)
    e_list = exp_sub.add_parser("list", help="List registered experiments")
    e_list.set_defaults(func=__import__("dualstream.experiment", fromlist=["main_cli"]).main_cli)
    e_val = exp_sub.add_parser("validate-protocol", help="Validate an experiment protocol")
    e_val.add_argument("--protocol", required=True)
    e_val.set_defaults(func=__import__("dualstream.experiment", fromlist=["main_cli"]).main_cli)
    e_run = exp_sub.add_parser("run", help="Run an experiment")
    e_run.add_argument("--protocol", required=True)
    e_run.add_argument("--experiment", required=True)
    e_run.add_argument("--outdir", default="results/raw")
    e_run.add_argument("--token-count", type=int, default=128)
    e_run.add_argument("--repetitions", type=int, default=20)
    e_run.set_defaults(func=__import__("dualstream.experiment", fromlist=["main_cli"]).main_cli)
    e_hash = exp_sub.add_parser("verify-hashes", help="Verify result artifact hashes")
    e_hash.add_argument("--run-dir", required=True)
    e_hash.set_defaults(func=__import__("dualstream.experiment", fromlist=["main_cli"]).main_cli)
    e_sum = exp_sub.add_parser("summarize", help="Regenerate summaries from raw runs")
    e_sum.add_argument("--raw-root", default="results/raw")
    e_sum.add_argument("--output", default="results/summaries/exp01_summary.json")
    e_sum.set_defaults(func=__import__("dualstream.experiment", fromlist=["main_cli"]).main_cli)
    e_tab = exp_sub.add_parser("tables", help="Generate manuscript-ready machine-readable tables")
    e_tab.add_argument("--summary", default="results/summaries/exp01_summary.json")
    e_tab.add_argument("--output", default="results/tables/exp01_tables.json")
    e_tab.set_defaults(func=__import__("dualstream.experiment", fromlist=["main_cli"]).main_cli)

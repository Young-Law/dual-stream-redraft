    ...
    def generate(self,prompt:str,cfg:GenerationConfig)->Dict[str,Any]:
        ...
        with torch.no_grad():
            for token_index in range(cfg.max_new_tokens):
                ...
                concepts=[ConceptScore(concept_id=cid,score=float(sc)) for cid,sc in self._heuristic_concepts(prompt,top_ids,top_probs)] if cfg.enable_heuristics else []
        ...
        fb=FallbackRouter(max_retries=cfg.max_red_retries,strategy=cfg.fallback_strategy).decide(red_state=out.outcome in {"FAIL","FALLBACK"},retry_count=cfg.max_red_retries)
        if fb.action!='none':
